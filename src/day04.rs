...
fn run(input: Vec<Vec<String>>) -> (usize, usize) {
    input
        .into_iter()
        .map(|block| {
        ...
        .map(|p| ((p.is_valid_pt1() as usize), (p.is_valid_pt2() as usize)))
        .fold((0, 0), |(r1, r2), (p1, p2)| (r1 + p1, r2 + p2))
}
